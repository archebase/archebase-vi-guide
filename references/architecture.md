...
- `tokens/archebase.tokens.json`: semantic colors, type families, logo policy, and `待确认` fields.
...
- `scripts/validate_tokens.py`: rejects colors outside approved token set unless explicitly marked `待确认`.
